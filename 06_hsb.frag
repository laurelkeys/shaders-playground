...
vec3 hsb2rgb_smooth(in vec3 c) {    
    vec3 rgb = clamp(abs(mod(c.x * 6.0 + vec3(0.0, 4.0, 2.0),
                     ...
                     0.0,
                     1.0);
    rgb = smoothstep(0.0, 1.0, rgb); // cubic smoothing
    return c.z * mix(vec3(1.0), rgb, c.y);
}
...
void main() {
    ...
    // and y (0.0 - 1.0) to the brightness
    color = hsb2rgb_smooth(vec3(st.x, 1.0, st.y));
    // color = hsb2rgb(vec3(st.x, 1.0, st.y));
    // color = hsb2rgb(rgb2hsb(hsb2rgb(vec3(st.x, 1.0, st.y))));
